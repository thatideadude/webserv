...
{
    std::cout << "  parseHeaders called" << std::endl;
    ...
    // Parse using index - MORE RELIABLE
    size_t pos = 0;
...
bool	Request::parseBody(const std::string &raw_data)
{
	...
	if (!_headers_parsed)
    {
        std::cout << "  Headers not parsed yet!" << std::endl;
        return (false);
    }
	size_t header_end = raw_data.find("\r\n\r\n");
	if (header_end == std::string::npos)
	{
        std::cout << "  No \\r\\n\\r\\n found in data!" << std::endl;
		return (false);
	}
...
bool	Request::_parseRequestLine(const std::string &line)
{
	...
	{
		std::cout << "  ⚠️ Unknown method: " << _method << " (will handle anyway)" << std::endl;
		return (false);
	}
	return (true);
}
